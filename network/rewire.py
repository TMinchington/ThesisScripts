...
def shuffle_node(net_dic, ran_key1, ran_key2):
    ...
    if new_edge1_key in net_dic or new_edge2_key in net_dic:
...
def run_job(net_path, x_ls, net_dic_pro1, net_dic_mir1):
    ...
    for x in x_ls:
        ...
        if not os.path.isdir('{}/rewire/ran_map-{}'.format(net_path, x)):
            os.makedirs('{}/rewire/ran_map-{}'.format(net_path, x))

        outfile = open('{}/rewire/ran_map-{}/ran_map-{}.txt'.format(net_path, x, x), 'w')
        ...
        target_edges_pro = 1.2*net_pro_len
        target_edges_mir = 1.2*net_mir_len
