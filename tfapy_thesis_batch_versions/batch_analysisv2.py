...
def perform_wavelet_analysis_on_file(input_file):
    ...
    if not os.path.isdir(os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet')): # creates an output directory for the wavelet data
        os.makedirs(os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet'))
    ...
    for cell_name in cell_data_frame:
        ...
        sinc_cutoff = (len(this_signal) * sampling_interval)- sampling_interval
        max_period = (len(this_signal) * sampling_interval) -sampling_interval
        period_range = np.linspace(min_period, max_period, number_periods)
...
#             all_ridges[long_key] = pd.Series(this_ridge_data[key])
        ...
        power_spectrum_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','power_spectrum_' + 
                                                cell_name +'.xlsx')
        ...
        ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','ridge_' + 
                                                    cell_name +'.xlsx')
    ...
    full_file_name = ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet', 'all_wavelet_data.xlsx')
    periods_file_name = ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','all_periods.xlsx')
    periods_file_name_csv = ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','all_periods.csv')
    powers_file_name = ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','all_powers.xlsx')
    phases_file_name = ridge_file_name = os.path.join(folder_name, input_file.replace('.xlsx', '_data'), 'wavelet','all_phases.xlsx')
